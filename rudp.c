...
struct rudp {
    // note(yan): rudp_send放在这个队列里面，每一次update会放在send_history里面
    struct message_queue send_queue;  // user packages will send

    // note(yan): 按照recv_id排序
    struct message_queue recv_queue;  // the packages recv

    // note(yan): 按照tick_id排序, 同时也是msg_id单调有序的
...
// note(yan): 如果发送小的package(<GENERAL_PACKAGE), message都可以被回收使用
//  超过这个大小的，如果没有办法被下次使用，就会被释放掉
static struct message *new_message(struct rudp *U, const uint8_t *buffer,
                                   int sz) {
...
// noote(yan): array也按照id排序, array只有send_again, 保存要求重发的id.
...
// note(yan): send history按照tick_tid顺序存放，将过期的message放在free list里面
...
// note(yan):
// 接受到的消息插入到recv_queue里面，按照recv_id排序，维护min_recv_id和max_recv_id.
static void insert_message(struct rudp *U, int id, const uint8_t *buffer,
                           int sz) {
    if (id < U->recv_id_min) return;
...
// note(yan): 收到一个UDP，里面会存在多条逻辑消息，包括请求，重发，数据和心跳
    ...
    while (sz > 0) {
        ...
        switch (len) {
            ...
            case TYPE_MISSING:
                ...
                (len == TYPE_REQUEST ? add_request : add_missing)(
                    U, get_id(U, buffer));
                buffer += 2;
                sz -= 2;
                break;
...
static void pack_request(struct rudp *U, struct tmp_buffer *tmp, int id,
                         int tag) {
...
static void pack_message(struct rudp *U, struct tmp_buffer *tmp,
                         struct message *m) {
    int sz = GENERAL_PACKAGE - tmp->sz;
    if (m->sz > GENERAL_PACKAGE - 4) {
        if (tmp->sz > 0) new_package(U, tmp);
        // big package
        sz = 4 + m->sz;
...
static void request_missing(struct rudp *U, struct tmp_buffer *tmp) {
...
static void reply_request(struct rudp *U, struct tmp_buffer *tmp) {
    int i;
...
static void send_message(struct rudp *U, struct tmp_buffer *tmp) {
...
struct rudp_package *rudp_update(struct rudp *U, const void *buffer, int sz,
                                 int tick) {
    U->current_tick += tick;
    clear_outpackage(U);
